...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_to_indices = self.words2charindices(sents)
        word_to_indices = pad_sents_char(sent_to_indices, self.char2id['<pad>'])
        tensor_output = torch.tensor(word_to_indices)
        tensor_output = tensor_output.permute(1,0,2)
