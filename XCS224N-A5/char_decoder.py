...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        embed_output = self.decoderCharEmb(input)

        # last_layer_all_t, all layers_last_t
        output,             (h_n, c_n) = self.charDecoder(embed_output, dec_hidden) # output shape: (length, batch, self.hidden_size)

        s_t = self.char_output_projection(output)  # shape: (length, batch, self.vocab_size)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        # 1: because we are skipping the pad_token. Now, 
        # This char_sequence[1:].contiguous().view(-1) should be list of indices of charater of interest 
